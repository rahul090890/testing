(function(e, t, n, r) {
	...
	var u = function(t, n) {
		...
		if (!t) {
			e('[data-growl="container"]').find('[data-growl="dismiss"]')
					.trigger("click")
		} else {
		...
		n.attr("data-growl-position", t.settings.placement.from + "-"
				+ t.settings.placement.align);
		...
		if (t.content.url) {
			e.find('[data-growl="url"]').attr("href", t.content.url).attr(
					"target", t.url_target);
		...
		e(
				'[data-growl-position="' + n.placement.from + "-"
						+ n.placement.align + '"]').each(
				function() {
					return r = Math.max(r, parseInt(e(this).css(
							n.placement.from))
							+ e(this).outerHeight() + n.spacing)
				});
		...
		if (t.delay >= 1) {
			...
			var r = setInterval(function() {
				...
				if (!e.hasClass("hovering") && t.mouse_over == "pause"
						|| t.mouse_over != "pause") {
	...
	f.prototype = {
		update : function(e, t) {
			...
			case "icon":
				if (this.settings.icon_type.toLowerCase() == "class") {
					this.$template.find('[data-growl="icon"]').removeClass(
					...
					if (this.$template.find('[data-growl="icon"]').is("img")) {
						...
						this.$template.find('[data-growl="icon"]').find("img")
								.attr().attr("src", t)
					}
				}
				break;
		...
		close : function() {
			var t = this.$template, n = this.settings, r = t
					.css(n.placement.from), i = false;
			if (n.onHide) {
				n.onHide(event)
			}
			t.addClass(this.settings.animate.exit);
			t.nextAll(
					'[data-growl-position="' + this.settings.placement.from
							+ "-" + this.settings.placement.align + '"]').each(
					function() {
